...
4. **Backend**: `api/`, `*.py`, `requirements.txt`, `vercel.json` — **sekarang DIBUKA** per user (2026-07-02: "gausah frontend only, backend juga gapapa"). Tapi fix scipy-reactivation ini **frontend-only** — `api/stats.py` gak diubah (cuma dibaca); auth header yg sudah diharuskan backend tinggal dipasang di frontend. Vercel deploy ditangani user sendiri.
...
### Test suite
- **420/423 pass** (3 skipped di `speaking-assessment-normality.test.js`). 0 fail.
- 9 fail pre-existing di `src/lib/stats/ttest.js` (Cohen's d & CI95 undefined) **sudah resolved** — `ttest.test.js` 9/9 hijau.
- +21 test baru dari normalizer scipy (`tests/statistics/normalizeScipy.test.js`).
- AGENTS.md klaim "366/366" — sekarang realita 420/423 (3 skip), sudah lewat itu.
...
- **2026-07-02 (phase 1: hasil Statistik jadi page baru + login → home)**: Dua perubahan. (1) **Login redirect ke home** — `Auth.jsx` default redirect setelah login/register/OTP/OAuth dari `/dashboard` → `/` (5 spot: line 63 OAuth return, 89 login, 120 register, 152 OTP, 159 OAuth param). Logout tetap `/`. (2) **Hasil analisis Statistik jadi page terpisah** `/statistik/hasil` — sebelumnya `ResultDisplay` dirender inline di `/statistik` (numpuk dgn form upload, refresh = hilang, scroll gak clear). Sekarang: `Statistik.jsx` setelah analisis selesai → `localStorage.setItem(\"statistik_result\", JSON.stringify(finalResult))` synchronous (bukan via usePersist effect, karena component unmount setelah navigate) + `navigate(\"/statistik/hasil\")`. Page baru `src/pages/StatistikHasil.jsx` baca via shared hook `usePersist(\"statistik_result\")` → render `ResultDisplay` (lazy-loaded, chunk terpisah). Empty state (user buka /statistik/hasil tanpa analisis) → redirect ke /statistik. Tombol \"Analisis Lain\" → navigate /statistik (result tetap persisted); \"Upload Ulang\" → clear localStorage + navigate /statistik. Extract `usePersist` (sebelumnya lokal di Assessment.jsx:177) ke shared `src/hooks/usePersist.js` buat dipakai cross-page. Statistik.jsx cleanup: hapus import `lazy`/`Suspense`/`ResultDisplay` (pindah ke StatistikHasil), hapus `handleBackToAnalysis` (dead code), ganti inline result render dgn comment. PageHeader StatistikHasil konsisten dgn family statistik (variant hero + subNav + breadcrumbs). Chunk Statistik 1,031.52 → 1,029.05 kB (ResultDisplay pindah ke chunk StatistikHasil). Build OK, 390/399 test (9 pre-existing ttest, 0 regresi).
- **2026-07-02 (phase 2: hasil EFA/Logistic/Mediation/ItemAnalysis jadi page baru)**: Lanjutan phase 1 — semua tool sekarang render hasil di page terpisah, bukan inline dgn form. Pola sama: tool page setelah analisis → `localStorage.setItem("<tool>_result", JSON.stringify(result))` synchronous + `navigate("/<tool>/hasil")`; page hasil baca via `usePersist("<tool>_result")`. 4 page baru: `src/pages/EFAHasil.jsx` (`/efa/hasil`), `src/pages/LogisticHasil.jsx` (`/logistik/hasil`), `src/pages/MediationHasil.jsx` (`/mediasi/hasil`), `src/pages/ItemAnalysisHasil.jsx` (`/butir-soal/hasil`). Komponen result (tables/plots/report-text) dipindah dari tool page ke hasil page; tool page dibersihkan dari inline result render, dead components, dead helpers, & import unused (-1366 baris total di 4 tool page, +38 baris di App.jsx buat 4 lazy import + 4 route). Detail per tool: (1) **ItemAnalysis** — hapus `result`/`aiInterpretation` state, `exportCSV`, dead components (ResultPanel/SummaryCard/DecisionPill/ItemRow/CategoryBadge/DecisionBadge) + dead helpers (reliabilityColor/labelReliability); keep HelpSection/parseMatrix/SAMPLE_* buat form input. (2) **EFA** — hapus `result`/`aiInterpretation` state + dead components (SamplingAdequacySection/ScreePlotSection/VarianceTableSection/LoadingsSection/ReportText); keep parseCSV/SAMPLE_CSV. (3) **Logistic** — hapus `result`/`aiInterpretation` state + dead components (CoefficientsTable/ModelFitSection/ClassificationSection/ROCSection/HosmerLemeshowSection/ReportText/Metric) + helper `classificationTable`; **threshold slider interactivity dipertahankan** — `cmAtThreshold` useMemo + `classificationTable()` recompute di hasil page (slider tetap hidup di page hasil). (4) **Mediation** — hapus `result`/`aiInterpretation` state + dead components (MediationResult/PathRow/PathDiagram/Box/PathArrow/ReportTextMediation/ModerationResult/SimpleSlopePlot/CondRow/ReportTextModeration); **dual-model branch** — hasil page cek `result._model === "mediation"` vs `"moderation"` (label "Mediasi"/"Moderasi") buat render komponen yg sesuai. Setiap hasil page: PageHeader dgn breadcrumbs (mis. "EFA › Hasil"), empty state dgn tombol ke tool, tombol "Analisis Baru" (clear localStorage + navigate tool) + "Analisis Lain". Catatan: teks JSX dgn angle bracket di-fix pakai HTML entity `<`/`>` (LogisticHasil) atau unicode `≥`/`→`/`−`/subscript (EFAHasil/MediationHasil) — raw angle bracket di JSX text content bikin build fail. Build OK, 390/399 test (9 pre-existing ttest, 0 regresi).
- **2026-07-02 (fix layout hasil Statistik: header duplikat + 8 tombol + TOC sticky)**: User komplain `/statistik/hasil` "tata letaknya masih jelek, tumpang tindih". Audit `ResultDisplay.jsx` nemu 3 masalah. (1) **Header duplikat** — card render `h3 "Hasil: {toolName}"` + `p "{sampleSize} sampel · {analyzedAt}"`, padahal `StatistikHasil` PageHeader tagline sudah nampilin `{toolName} · {sampleSize} sampel · {analyzedAt}` yg sama. (2) **8 tombol wrap berantakan** — Simpan + Excel + PDF + DOCX + R Syntax + 📝 Laporan AI + Analisis Lain + Upload Ulang, lebar beda-beda, di mobile wrap 3-4 baris. (3) **TOC sticky rusak** — card punya `overflow-hidden` (buat clip rounded corners), tapi `overflow-hidden` bikin `position: sticky` TOC bar (z-20, top=headerH) gak nempel ke viewport — TOC malah scroll/lompat erratic. Fix: (1) hapus h3+p duplikat; toolbar jadi baris aksi rata kanan. (2) refactor `ExportActions.jsx` — 5 tombol export (Excel/PDF/DOCX/R/Laporan AI) digabung jadi 1 tombol "Export ▾" + dropdown menu (outside-click + Escape close, theme tokens, z-50). Toolbar sekarang 4 tombol seragam sizing (text-xs sm:text-sm, px-3 sm:px-4): Simpan | Export ▾ | Analisis Lain (primary bg-accent) | Upload Ulang — muat 1-2 baris di mobile. (3) hapus `overflow-hidden` dari card — aman karena `animate-fade-in-up` gak punya `fill-mode` (transform gak persist setelah 0.4s), & children (toolbar no-bg / TOC bg-card sama dgn card / content no-bg) gak poke rounded corners. TOC sticky sekarang beneran nempel di bawah PageHeader. Build OK, 390/399 test (9 pre-existing ttest, 0 regresi).
- **2026-07-02 (phase 3: hasil EViews jadi page baru)**: Penutup fase pemisahan hasil — EViews (tool terakhir yg masih render hasil inline) sekarang punya page hasil terpisah `/eviews/hasil`. Sebelumnya EViews punya 3 tab (Estimasi Panel | Diagnostik | Time Series) dgn semua hasil dirender inline. Sekarang: tool page jadi **2 tab** (Estimasi Panel | Time Series) — input/config only; tab Diagnostik dihapus (uji diagnostik dipindah ke page hasil sebagai section interaktif). Pola persist+navigate sama dgn phase 1-2: handler compute → `localStorage.setItem("eviews_result", JSON.stringify(payload))` synchronous + `navigate("/eviews/hasil")`. Page baru `src/pages/EViewsHasil.jsx` baca via `usePersist("eviews_result")`, branch pada `result._type`: `"estimasi"` → ModelSummaryCard + CoefficientsTable + Hausman (tombol interaktif → `hausmanTestAdapter`) + Diagnostik (3 tombol interaktif BP/White/Wooldridge → adapters recompute pakai persisted `data`+`panelConfig`); `"timeseries"` → ADF cards + LagInfoCard / Granger cards + warnings / Cointegration card. Detail: (1) **handleEstimate** compute ke local var `est` (pooledOLS/FE/RE + auto-counterpart for Hausman), persist `{_type:"estimasi", data, columns, fileName, estimationResults, panelConfig:cfg, diagnosticResults:{all null}, analyzedAt}`. (2) **runADF/runGranger/runCoint** persist `{_type:"timeseries", data, columns, fileName, tsConfig, tsResults:{...}, analyzedAt}`. (3) **Granger stationarity check** — sebelumnya baca `tsResults.adf` dari state; sekarang baca last ADF results dari `localStorage.getItem("eviews_result")`. (4) **State restoration** — tool page restore `data`+`columns`+`fileName`+`tsConfig` dari `eviews_result` via lazy `useState` init (`useMemo` read sekali di mount), jadi user balik dari hasil page bisa run test lain tanpa re-upload. Upload baru / `clearAll` → `localStorage.removeItem("eviews_result")`. (5) Tab Diagnostik dihapus; Hausman + BP/White/Wooldridge jadi tombol interaktif di hasil page. Tool page cleanup: hapus import result components (ModelSummaryCard/CoefficientsTable/HausmanCard/DiagnosticsCard + ADFResultCard/GrangerResultCard/CointegrationResultCard/LagInfoCard) + diagnostic adapters (hausmanTestAdapter/breuschPaganAdapter/whiteTestAdapter/wooldridgeTestAdapter) + icons (Activity/Settings2); hapus state (panelConfig/estimationResults/diagnosticResults/tsResults/tsRows) + helpers (runHausman/runDiagnostic). `App.jsx`: +1 lazy import `EViewsHasil` + 3 route (`/eviews/hasil`, `/panel-data/hasil`, `/time-series/hasil`). Chunk EViews tool page 64.47 kB; chunk lazy `EViewsHasil` 29.42 kB. Build OK, 390/399 test (9 pre-existing ttest, 0 regresi).
- **2026-07-02 (scipy-reactivation: fix auth + normalizer merge)**: **scipy backend sebelumnya dead code di production.** Root cause: `callPythonBackend` (`src/lib/stats/backend.js`) gak kirim `Authorization: Bearer <JWT>` — padahal `api/stats.py:106-114` mewajibkan JWT di setiap POST. Health-check GET (`isPythonBackendAvailable()`) gak butuh auth → return true → POST → 401 → throw → JS fallback jalan. Hasil: scipy gak pernah dipakai; semua analisis pakai JS adapter. Fix **frontend-only** (`api/stats.py` gak diubah, cuma dibaca): (1) **Auth header** — `callPythonBackend` sekarang `supabase.auth.getSession()` → throw kalau no `session.access_token` → header `Authorization: Bearer <token>` (pola sama `wallet.js:127-132`). (2) **Error surfacing** — scipy return `{success:true, result:{error}}` 200-envelope buat compute error; sebelumnya ke-swallow (anggap sukses) → result `{error}` masuk ke ResultCards. Sekarang di-surface sebagai throw → JS fallback jalan. (3) **JS fallback semua 17 `*Backend`** — sebelumnya 11 function throw `new Error('Backend unavailable')` tanpa fallback (user lihat error layar); sekarang semua `try scipy → normalize → catch → withJsBackend(adapterJS(...))`. (4) **Payload fix oneSample** — kirim `{mode:'oneSample', sample:values, popmean:mu0}` (scipy baca `data['sample']`/`data['popmean']`, bukan `values`/`mu0`). (5) **Overclaim fix** — `getBackendStatus` recommendation "100% SPSS accurate" → "scipy + statsmodels". Modul baru **`src/lib/stats/normalizeScipy.js`** (17 normalizer, ~250 baris) — strategi **merge**: adapter JS di-run di input yg sama buat shape, field inferensial di-override nilai scipy, field yg missed keep nilai JS (gak pernah `undefined`), tag `backend:'scipy'`. Cocok dgn preferensi user "scipy primary, JS 1-2% fallback" — scipy override field-by-field, JS isi gap. Mapping field: scipy `effectSize`→adapter `cohensD`; scipy `ci95:{low,high}`→adapter `ci95:[lo,hi]` (via helper `ciArray()`); scipy `epsilonSquared`→`etaSquared` (Kruskal); scipy `intercept`/`slope`→`b0`/`b1`; scipy `rSquared`→`r2` & `adjRSquared`→`adjustedR2`; scipy `pValue`(F)→`pF`; scipy `coef`/`pValue`(coeffs)→`b`/`p`. Two-way ANOVA: overlay scipy `effects[]` F/df/p/η² ke `anovaTable[]` adapter. Normality: force shape Shapiro-Wilk (method + W statistic) walau adapter pilih KS buat n>50. Test baru `tests/statistics/normalizeScipy.test.js` (21 test, fixture scipy dgn nilai distinctive mis. t=2.345) — cover: scipy value override, ci95 object→array, adapter shape preservation (means/sds/welch/vifs/durbinWatson), error passthrough, backend tag. Full suite **420/423** (0 fail, 3 skip) — baseline 399 + 21 baru. Build OK (3313 modules, PWA precache 20); **catatan Windows env**: butuh `NODE_OPTIONS=--max-old-space-size=4096` (oxc transformer OOM di heap default Node). Konsumen (ResultCards/export/AI/Statistik.jsx) gak diubah — merge strategy jaga shape kompatibel.
