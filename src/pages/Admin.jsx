      ...
      <header className="bg-white/80 backdrop-blur-md shadow-sm sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={handleLogout} className="flex items-center gap-2 text-muted hover:text-red-600">
        ...
        <div className="bg-card rounded-2xl shadow-sm overflow-hidden mb-6">
          ...
          <div className="overflow-x-auto">
            {pendingTopups.length === 0 ? (
              <p className="p-6 text-center text-sm text-muted">Belum ada permintaan top-up manual.</p>
            ) : (
              <table className="w-full">
                <thead className="bg-surface">
                  <tr>
                    <th className="px-4 py-3 text-left text-xs font-medium text-muted">Order ID</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-muted">Email</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-muted">Nominal</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-muted">Bonus</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-muted">Metode</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-muted">Waktu</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-muted">Status</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-muted">Aksi</th>
                  </tr>
                </thead>
                <tbody className="divide-y">
                  {pendingTopups.slice(0, 30).map(p => (
                    <tr key={p.id} className="hover:bg-surface">
                      ...
                      <td className="px-4 py-3 text-xs text-muted">{p.submittedAt}</td>
            ...
            <table className="w-full">
              <thead className="bg-surface">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-muted">Amount</th>
              ...
              <tbody className="divide-y">
                {filteredTransactions.map(t => (
                  <tr key={t.id} className="hover:bg-surface">
                    ...
                    <td className="px-4 py-3 text-sm text-muted">{t.date}</td>
                  </tr>
                ))}
              </tbody>
            </table>
