            ...
            <div key={idx} className="bg-card border border-border/80 rounded-xl p-4">
              {it ? (
                <>
                  ...
                  <h2 className="font-bold text-fg text-base mb-1.5 truncate" title={it.title}>{it.title}</h2>
          ...
          <div className="mt-5 grid grid-cols-2 gap-4">
            {[a, b].map((it, idx) => (
              <div key={idx} className="bg-card border border-border/80 rounded-xl p-4">
                ...
                {it?.notes ? (
                  <div className="text-sm text-fg whitespace-pre-wrap">{it.notes}</div>
                ) : (
...
function CellValue({ item }) {
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap">
      <span className={item.sig ? 'text-teal font-semibold' : 'text-fg'}>
        {item.value}
      </span>
