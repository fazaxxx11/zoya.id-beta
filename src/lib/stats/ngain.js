...
// =====================================
// Mengukur efektivitas pembelajaran/treatment dari pre-test → post-test.
//
// Formula:
...
export function calcNGain(pre, post, maxScore = 100) {
  ...
  if (denom <= 0) {
    // Pre-test sudah max — kategorikan sebagai tidak bisa diukur
    return post >= pre ? 1.0 : 0.0
  }
  const g = (post - pre) / denom
  // Clamp untuk handle outlier (post < pre dengan pre tinggi)
 ...
 * @param {number} args.maxScore - skor maksimum (default 100)
...
export function analyzeNGain({ pre, post, maxScore = 100, names = [] }) {
  ...
  try {
    ...
    if (tt && !tt.error) {
      signifTest = {
        ...
        df: tt.df,
        pValue: tt.pValue,
        meanDiff: tt.meanDiff,         // positif = peningkatan
        cohensD: tt.cohensD,
  ...
  return {
    ...
    n: pairs.length,
    maxScore,
    pairs,            // detail per individu (sudah ada kategori)
